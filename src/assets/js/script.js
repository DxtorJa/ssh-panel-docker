...
function repairDropbear(ip)
{
	...
	$.ajax({
		...
		success: function(data)
		{
			...
			$('#result').append('<div class="alert alert-success">L2TP successfully repaired, Please refresh this pages to see the result.</div>');
...
function testDropbear(ip)
{
	...
	$.ajax({
		...
		success: function(data)
		{
			...
			$('#result').append('<div class="alert alert-success">L2TP Connected successfully!</div>');
			$('#btn-test-dropbear').text('TEST');
			$("html, body").animate({ scrollTop: 0 }, "slow");
			return showNotification('success', 'L2TP Test: Running...');
		...
		$.ajax({
			...
			success: function(d)
			{
				...
				if(d.status != 'Trial Success')
				{
					...
					// display result
					var result = $('<div class="alert alert-success"><h4>VPN Account successfully created!</h4><br /> Username: <code>' + d.details.username + '</code> <br /> Password: ' + d.details.password + ' <br /> Host/IP: ' + d.details.host + ' <br /> IPSec PSK:   ' + d.details.psk + ' <br /> Message: '+ d.message +'</div>').hide().fadeIn('slow');
					$('div#result').append(result);
					
					$("html, body").animate({ scrollTop: 0 }, "slow");			
					return showNotification('success', 'VPN Account Successfully created!');
				}
